...
let () =
  ...
  let ciphertext = Chacha20_simd.chacha20_crypt ~key ~nonce ~initial_counter:0 message in
  ...
  let decrypted = Chacha20_simd.chacha20_crypt ~key ~nonce ~initial_counter:0 ciphertext in
